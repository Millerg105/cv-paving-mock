...
export default function PorcelainPavingGallery() {
    ...
    const categoryImages = galleryImages.filter(src => src.toLowerCase().includes('porcelain'));
            ...
            <div className="pt-32 pb-16 px-6 md:px-12 flex-grow max-w-[1400px] mx-auto w-full">
                <div className="text-center mb-16">
                    ...
                    <div className="flex flex-wrap justify-center gap-4 mb-4">
                        ...
                        <Link href="/gallery/artificial-grass" className="px-6 py-2 rounded-full border border-white/20 hover:border-primary/50 text-foreground/70 hover:text-foreground uppercase text-sm tracking-widest transition-all">
                            Artificial Grass
                        </Link>
                        <Link href="/gallery/porcelain-paving" className="px-6 py-2 rounded-full border border-primary text-primary uppercase text-sm tracking-widest font-bold">
                            Porcelain Paving
                        </Link>
                ...
                {categoryImages.length > 0 ? (
                    <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 space-y-4">
                        {categoryImages.map((src, i) => (
                            <div key={i} className="relative w-full break-inside-avoid rounded-xl overflow-hidden shadow-lg border border-white/10 group">
                                <Image
                                    src={src}
                                    alt={`Porcelain Paving Project ${i + 1}`}
                                    width={800}
                                    height={600}
                    ...
                    <div className="text-center py-20 text-foreground/40">
                        Loading porcelain paving examples...
                    </div>
                )}
            </div>
